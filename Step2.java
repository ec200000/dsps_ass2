...
public class Step2 {
    ...
    // taking the 2-gram and counting number of times each word occurs.
    // each line from the 2-gram is:
    // ngram TAB year TAB occurrences TAB pages TAB books NEWLINE
    // Input and output for example: "Bait Gadol" 2000 5 1 1 => "Bait Gadol" 5

    private static class MapperClass extends Mapper<LongWritable, Text, Text, Text> {
        @Override
        public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            String[] vals = value.toString().split("\t");
            if (vals[0].split(" ").length > 1) {
                Text words = new Text();
                words.set(vals[0]);
    ...
    // Input and output For example: "Bait Gadol" 5 "Bait Gadol" 3 => "Bait Gadol" 8
